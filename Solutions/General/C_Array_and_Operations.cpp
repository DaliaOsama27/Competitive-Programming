...
const int MAXV = 3005; // set according to problem constraints
const int INF = 1e9;
...
void solve(int test_case) {
   ...
   for(int i = 0 ; i < n ; i++){
        ...
        fa[i] = fact(v[i]);
        if((i+1)&1){
            for(int a = 0 ; a < fa[i].size() ; a++){
                st.addEdge(n*30,i*30+a,1);
            }
        }
        else{
            for(int a = 0 ; a < fa[i].size() ; a++){
                st.addEdge(i*30+a,n*30+1,1);
            }
        }
   }
   for(int i = 0 ; i < m ; i++){
        ...
        if(y&1) swap(x,y); 
        x--,y--;
        for(int a = 0 ; a < fa[x].size() ; a++){
            for(int b = 0 ; b < fa[y].size() ; b++){
                if(fa[x][a] == fa[y][b]){
                    st.addEdge(x*30+a,y*30+b,1);
                }
            }
        }
   }
   cout<<st.maxFlow()<<ln;
   // st.printGraph(n*30+2);
}
